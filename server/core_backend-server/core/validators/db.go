...
func NormalizeUniqueIndexError(err error, tableOrAlias string, fieldNames []string) error {
	...
	*/
	...
	// MySQL:
	if me, ok := err.(interface{ Number() uint16 }); ok && me.Number() == 1062 {
		msg := strings.ToLower(err.Error())
		normalizedErrs := validation.Errors{}

		for _, name := range fieldNames {
			// for MySQL we check if the index name or column name is in the error message
			// (usually it contains the index name which by default in PocketBase includes the column name)
