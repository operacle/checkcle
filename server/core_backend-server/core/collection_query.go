...
func (app *BaseApp) FindAllCollections(collectionTypes ...string) ([]*Collection, error) {
	...
	if IsPostgres(app) {
		// PostgreSQL: Sorting by ctid to provide a relatively more stable result.
		q.OrderBy("ctid ASC")
	} else if IsMysql(app) {
		// MySQL: Sorting by id.
		q.OrderBy("id ASC")
	} else {
		// SQLite: Sorting by rowid.
		q.OrderBy("rowid ASC")
	}
...
func (app *BaseApp) FindCollectionByNameOrId(nameOrId string) (*Collection, error) {
	...
	if err != nil && errors.Is(err, sql.ErrNoRows) && strings.HasPrefix(nameOrId, "pbc_") {
		// try with obc_ prefix for compatibility
		altId := "obc_" + nameOrId[4:]
		err = app.CollectionQuery().
			AndWhere(dbx.NewExp("[[id]]={:id}", dbx.Params{"id": altId})).
			Limit(1).
			One(m)
	}
...
func (app *BaseApp) FindCachedCollectionByNameOrId(nameOrId string) (*Collection, error) {
	...
	if strings.HasPrefix(nameOrId, "pbc_") {
		altId := "obc_" + nameOrId[4:]
		for _, c := range collections {
			if c.Id == altId {
				return c, nil
...
func saveViewCollection(app App, newCollection, oldCollection *Collection) error {
	...
	return app.RunInTransaction(func(txApp App) error {
		...
		if !IsPostgres(txApp) {
			// SQLite: delete old renamed view
...
func normalizeViewQueryId(app App, query string) (string, error) {
	...
	if err := rawParsed.parse(app, query); err != nil {
		return "", err
	}
