...
// IsPostgres checks if the current app database is PostgreSQL.
func IsPostgres(app App) bool {
	if base, ok := app.(*BaseApp); ok {
		if base.driverName != "" {
			return base.driverName == "postgres"
		}
		detectDriver(base)
		return base.driverName == "postgres"
	}
	...
	var version string
	err := app.DB().NewQuery("SELECT version()").Row(&version)
	return err == nil && strings.Contains(strings.ToLower(version), "postgres")
}

// IsMysql checks if the current app database is MySQL.
func IsMysql(app App) bool {
	if base, ok := app.(*BaseApp); ok {
		if base.driverName != "" {
			return base.driverName == "mysql"
		}
		detectDriver(base)
		return base.driverName == "mysql"
	}
...
func detectDriver(app *BaseApp) {
	if app.concurrentDB == nil {
		return
	}

	var version string
	err := app.concurrentDB.NewQuery("SELECT version()").Row(&version)
	if err != nil {
		app.driverName = "sqlite"
		return
	}
