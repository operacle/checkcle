...
func (m *Collection) AddIndex(name string, unique bool, columnsExpr string, optWhereExpr string) {
	...
	if dbType == "mysql" {
		...
		idx.WriteString(")")
		// MySQL doesn't support partial index WHERE clause
...
func (c *Collection) idChecksum() string {
	return "obc_" + crc32Checksum(c.Type+c.Name)
}
...
func (c *Collection) initEmailField() {
	...
	if _, ok := dbutils.FindSingleColumnUniqueIndex(c.Indexes, FieldNameEmail); !ok {
		format := `CREATE UNIQUE INDEX "%s" ON "%s" ("%s") WHERE "%s" != ''`
		dbType := os.Getenv("DB_TYPE")
		var args []any = []any{
		...
		if dbType == "mysql" {
			...
			format = "CREATE UNIQUE INDEX `%s` ON `%s` (`%s`) WHERE `%s` != ''"
			args = append(args, FieldNameEmail)
		} else {
			// postgres
			args = append(args, FieldNameEmail)
		}
		c.Indexes = append(c.Indexes, fmt.Sprintf(format, args...))
	}
}
