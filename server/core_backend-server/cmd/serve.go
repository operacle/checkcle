...
func NewServeCommand(app core.App, showStartBanner bool, hideAdminUI bool) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Short:        "Starts the web server (default to 127.0.0.1:8198 if no domain is specified)",
		SilenceUsage: true,
	...
	command.PersistentFlags().StringVar(
		...
		"TCP address to listen for the HTTP server\n(if domain args are specified - default to 0.0.0.0:80, otherwise - default to 127.0.0.1:8198)",
	)
