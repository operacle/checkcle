...
func NewTestApp(optTestDataDir ...string) (*TestApp, error) {
	...
	// docker run -d --rm --name postgres -e POSTGRES_USER=user -e POSTGRES_PASSWORD=pass -p 6432:6432 postgres:alpine
	return NewTestAppWithConfig(core.BaseAppConfig{
		DataDir:       testDataDir,
		EncryptionEnv: "pb_test_env",
		PostgresURL:   "postgres://user:pass@127.0.0.1:6432/postgres?sslmode=disable",
