...
type Config struct {
	...
	DefaultPostgresURL    string // if not set, it will fallback to "postgres://user:pass@127.0.0.1:6432/postgres?sslmode=disable"
...
func NewWithConfig(config Config) *PocketBase {
	...
	if config.DefaultPostgresURL == "" {
		config.DefaultPostgresURL = os.Getenv("POSTGRES_URL")
	}
	if config.DefaultPostgresDataDb == "" {
		config.DefaultPostgresDataDb = os.Getenv("POSTGRES_DATA_DB")
	}
	if config.DefaultPostgresAuxDb == "" {
		config.DefaultPostgresAuxDb = os.Getenv("POSTGRES_AUX_DB")
	}
	...
	if os.Getenv("PB_ADMIN_UI_ENABLED") == "false" {
		config.HideAdminUI = true
	}
	...
	pb := &PocketBase{
		...
		hideAdminUI:        config.HideAdminUI,
	}
...
func (pb *PocketBase) Start() error {
	...
	pb.RootCmd.AddCommand(cmd.NewServeCommand(pb, !pb.hideStartBanner, pb.hideAdminUI))
