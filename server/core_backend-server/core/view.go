...
func getCastRegex(app App) *regexp.Regexp {
	if IsPostgres(app) {
		return castRegex
	}
	return regexp.MustCompile(`(?is)^cast\s*\(.*\s+as\s+(\w+)\s*\)$`)
}

func parseQueryToFields(app App, selectQuery string) (map[string]*queryField, error) {
	p := new(identifiersParser)
	if err := p.parse(app, selectQuery); err != nil {
		return nil, err
	}
	...
	for _, col := range p.columns {
		...
		castMatch := getCastRegex(app).FindStringSubmatch(colLower)
		if castMatch == nil && IsPostgres(app) {
...
func (p *identifiersParser) parse(app App, selectQuery string) error {
	...
	selects, err := extractIdentifiers(app, selectParts.String())
	if err != nil {
		return err
	}

	froms, err := extractIdentifiers(app, fromParts.String())
	if err != nil {
		return err
	}

	joins, err := extractIdentifiers(app, joinParts.String())
	if err != nil {
		return err
	}
...
func extractIdentifiers(app App, rawExpression string) ([]identifier, error) {
	...
	for _, rawIdentifier := range rawIdentifiers {
		...
		resolved, err := identifierFromParts(app, parts)
		if err != nil {
			return nil, err
		}
...
func identifierFromParts(app App, parts []string) (identifier, error) {
	if !IsPostgres(app) {
		return identifierFromParts_sqlite(parts)
	}
	return identifierFromParts_postgres(parts)
}
...
// PostgreSQL:
func identifierFromParts_postgres(parts []string) (identifier, error) {
	if len(parts) == 1 {
		// eg: `id`, `table_1.id`
		subParts := strings.Split(parts[0], ".")
		alias := subParts[len(subParts)-1]
		return identifierFromParts_postgres([]string{parts[0], alias})
	}
