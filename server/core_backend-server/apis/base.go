...
func NewRouter(app core.App) (*router.Router[*core.RequestEvent], error) {
	...
	bindSqlEditorApi(app, apiGroup)
	bindEdgeFunctionsApi(app, apiGroup)

	// Add /api/v1 versioned endpoints for public API access
	// This provides a versioned API for external clients while keeping
	// the admin UI using /api routes
	apiV1Group := pbRouter.Group("/api/v1")
	bindCollectionApi(app, apiV1Group)
	bindRecordCrudApi(app, apiV1Group)
	bindRecordAuthApi(app, apiV1Group)
	bindFileApi(app, apiV1Group)
	bindBatchApi(app, apiV1Group)
	bindRealtimeApi(app, apiV1Group)
	bindHealthApi(app, apiV1Group)
