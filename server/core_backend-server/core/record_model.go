...
func (record *Record) PublicExport() map[string]any {
	...
	if !hasCustomVisibility || isVisible {
		// Replace pbc_ prefix with obc_ for OrchBase branding
		collectionId := record.collection.Id
		if strings.HasPrefix(collectionId, "pbc_") {
			collectionId = "obc_" + strings.TrimPrefix(collectionId, "pbc_")
		}
		export[FieldNameCollectionId] = collectionId
	}
