...
type BaseAppConfig struct {
	...
	AuxMaxOpenConns  int
	AuxMaxIdleConns  int
	PostgresURL      string // eg: "postgres://user:pass@localhost:6432?sslmode=disable"
...
func NewBaseAppForTest(config BaseAppConfig) (*BaseApp, func()) {
	...
	exec.Command("sh", "-c", "PGPASSWORD=pass dropdb -h localhost -p 6432 -U user "+testAuxDB).Run()
	exec.Command("sh", "-c", "PGPASSWORD=pass dropdb -h localhost -p 6432 -U user "+testDataDB).Run()
	exec.Command("sh", "-c", "PGPASSWORD=pass createdb -h localhost -p 6432 -U user "+testAuxDB).Run()
	exec.Command("sh", "-c", "PGPASSWORD=pass createdb -h localhost -p 6432 -U user "+testDataDB).Run()

	config.PostgresURL = "postgres://user:pass@localhost:6432/postgres?sslmode=disable"
	...
	cleanup := func() {
		...
		defer exec.Command("sh", "-c", "PGPASSWORD=pass dropdb -h localhost -p 6432 -U user "+app.config.PostgresDataDB).Run()
		defer exec.Command("sh", "-c", "PGPASSWORD=pass dropdb -h localhost -p 6432 -U user "+app.config.PostgresAuxDB).Run()
	}
...
func (app *BaseApp) initDataDB() error {
	...
	app.concurrentDB = concurrentDB // fix: assign immediately to allow driver detection
	...
	var nonconcurrentDB *dbx.DB
	if IsPostgres(app) {
		nonconcurrentDB = concurrentDB
	} else {
		// SQLite:
		// Note: SQLite does not allow multiple write connections at the same time.
		// So we use a separate connection with max open connections limited to 1.
		db, err := app.config.DBConnect(app.config.PostgresDataDB)
		if err != nil {
			return err
		}
	...
	if app.IsDev() {
		concurrentDB.QueryLogFunc = func(ctx context.Context, t time.Duration, sql string, rows *sql.Rows, err error) {
			if err != nil {
				color.HiRed("[%.2fms] %v, err: %v\n", float64(t.Milliseconds()), normalizeSQLLog(sql, IsPostgres(app)), err)
			} else {
				color.HiBlack("[%.2fms] %v\n", float64(t.Milliseconds()), normalizeSQLLog(sql, IsPostgres(app)))
			}
		}
		...
		nonconcurrentDB.QueryLogFunc = concurrentDB.QueryLogFunc
	}
...
var sqliteLogReplacements = []struct {
...
var postgresLogReplacements = []struct {
	pattern     *regexp.Regexp
	replacement string
}{
	{regexp.MustCompile(`\[\[([^\[\]\{\}\.]+)\.([^\[\]\{\}\.]+)\]\]`), `"$1"."$2"`},
...
func normalizeSQLLog(sql string, isPostgres bool) string {
	replacements := sqliteLogReplacements
	if isPostgres {
		replacements = postgresLogReplacements
	}
...
func (app *BaseApp) initAuxDB() error {
	...
	app.auxConcurrentDB = concurrentDB // fix: assign immediately to allow driver detection
	...
	if IsPostgres(app) {
		...
		// Note: SQLite does not allow multiple write connections at the same time.
		// So we use a separate connection with max open connections limited to 1.
		db, err := app.config.DBConnect(app.config.PostgresAuxDB)
		if err != nil {
			return err
		}
		db.DB().SetMaxOpenConns(1)
